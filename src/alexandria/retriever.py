...
class HybridRetriever:
    ...
    Blends dense vectors and HippoRAG 2 PPR matrix routing.
    """
    def __init__(self, vault: AlexandriaVault, rrf_constant: int = 60):
        self.vault = vault
        self.k = rrf_constant

    def compute_rrf(self, vector_results: List[str], graph_chunk_ids: List[str]) -> List[str]:
    ...
    async def retrieve_context(self, query_vector: List[float], seed_entities: List[str], limit: int = 5) -> str:
        ...
        # FIX: Delegate graph search to the new SciPy Matrix Engine
        graph_hits = await self.vault.matrix_engine.execute_ppr(seed_entities, top_k=20)
